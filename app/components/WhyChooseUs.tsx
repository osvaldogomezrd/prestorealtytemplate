      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-14">
          {highlights.map((h, i) => (
            <div
              key={i}
              className="bg-[#1a2744] rounded-2xl p-6 text-center text-white"
            >
              <div className="text-3xl font-extrabold text-red-400 mb-1">{h.value}</div>
              <div className="text-sm text-gray-300">{h.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="text-center mt-10">
          <a
            href="#contact"
            className="inline-block bg-red-600 hover:bg-red-700 text-white font-semibold px-10 py-4 rounded-lg transition-colors duration-200 shadow-md"
          >
